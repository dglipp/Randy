...
bool initVulkan()
{
	...
	{
		std::cout << "Cannot create data buffers" << std::endl;;
		exit(1);
	}
	...
	if (!createDescriptorPool(vkDev, 1, 2, 1, &vkState.descriptorPool) ||
		...
		!createGraphicsPipeline(vkDev, vkState.renderPass, vkState.pipelineLayout, { "../Tests/TestVulkan/shaders/VK.frag", "../Tests/TestVulkan/shaders/VK.geom", "../Tests/TestVulkan/shaders/VK.vert" }, &vkState.graphicsPipeline))
	{
...
int main()
{
	...
	window = glfwCreateWindow(kScreenWidth, kScreenHeight, "Vulkan App", nullptr, nullptr);
	if (!window)
	{
		glfwTerminate();
		exit(EXIT_FAILURE);
