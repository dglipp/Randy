...
void CHECK(bool check, const char *fileName, int lineNumber)
{
    if (!check)
    {
        std::cout << "CHECK() failed at " << fileName << ":" << lineNumber << std::endl;
        assert(false);
        exit(EXIT_FAILURE);
    }
}
    ...
    void *UserData)
{
    ...
    std::cout << "Debug callback (" << pLayerPrefix << "): " << pMessage << std::endl;
    return VK_FALSE;
}
...
VkResult createShaderModule(VkDevice device, ShaderModule *shader, std::string fileName)
{
