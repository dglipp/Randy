...
b8 platform_startup(
    ...
    HWND handle = CreateWindowExA(window_ex_style, "randy_window_class", application_name, window_style, window_x, window_y, window_width, window_height, 0, 0, state->h_instance, 0);

    if (handle == 0) {
        MessageBoxA(NULL, "Window creation failed", "Error", MB_ICONEXCLAMATION | MB_OK);

        RFATAL("Window creation failed");
        return FALSE;
    } else {
        state->hwnd = handle;
    }

    b32 should_activate = 1; // TODO: if window should not accept input it should be false
